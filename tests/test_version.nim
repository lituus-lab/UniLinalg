...
suite "one version, six copies":
  let manifest = valueOf("UniLinalg.nimble", "version", "\"", "\"")
  ...
  test "the Nim constant agrees":
    check UniLinalgVersion == manifest

  test "the C header agrees, macros and string alike":
    let parts = manifest.split('.')
    check valueOf("include/UniLinalg.h", "UNILINALG_VERSION_MAJOR", " ",
        "") == parts[0]
    check valueOf("include/UniLinalg.h", "UNILINALG_VERSION_MINOR", " ",
        "") == parts[1]
    check valueOf("include/UniLinalg.h", "UNILINALG_VERSION_PATCH", " ",
        "") == parts[2]
    check valueOf("include/UniLinalg.h", "define UNILINALG_VERSION ", "\"",
        "\"") == manifest

  test "the C ABI reports it":
    # Read from the source rather than called: this suite links no C library.
    check valueOf("src/UniLinalg/c_api.nim", "UniLinalgVersionC", "\"",
        "\"") == manifest
  ...
  test "the Python test expects it":
    check valueOf("py/tests/test_linalg.py", "unilinalg.version()", "\"",
        "\"") == manifest
