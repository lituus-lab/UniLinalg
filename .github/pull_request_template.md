...
- [ ] `nimble pyTest` passes (if the Python binding or C ABI was touched)
- [ ] `nimble lint` and `nimble checkVGraph` pass
- [ ] `nimble example` runs (if the public Nim API was touched)
- [ ] `nimble coverage` and `nimble docs` still build (if touched)
- [ ] C ABI touched → `include/UniLinalg.h` updated in the same commit
